...
We have implemented Supervisionary as a WebAssembly (Wasm henceforth) host, which allows us to rapidly explore the most important ideas.
...
\paragraph{Robust and flexible theorem proving tools}

Immediately, we can improve the robustness and flexibility of familiar theorem proving software by restructuring it around the Supervisionary kernel.
For example, we can refactor a typical LCF-style proof assistant so that the source-level module implementing its proof-checking kernel acts as frontend for the Supervisionary kernel, proxying derivations across an isolation boundary such as a process boundary or Wasm sandbox boundary.
Before, we relied on the ML type system to protect the soundness of our system from errors outside of the kernel.
Now, we rely on a stronger system-level boundary, meaning that even memory corruption in the rich portion of our system does not pose a threat to its soundness.

Stepping further, we can design less familiar theorem proving systems where the rich portion itself contains internal system-level isolation boundaries.
For example, under a single instance of the Supervisionary kernel, a group of distinct theorem proving clients could construct a proof in concert.
Furthermore, in certain cases, isolating specification from proof might allow for more confidence in the correctness of a specification's expression.

% note: explicitly mention polyglot theorem proving software?
